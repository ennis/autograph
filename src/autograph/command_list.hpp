...
public:
  CommandList(D& backend_, UploadBuffer<D>& upload_buf_,
              typename D::CommandListHandle handle_)
      : backend(backend_), upload_buf(upload_buf_), handle(handle_) {}
...
private:
  struct BindState {
    enum class TextureDimension { Tex1D, Tex2D, Tex3D };
    struct TexBindState {
