...
{ 
	...
	device.run([&]() {
		...
		u_scene.viewMatrix = glm::lookAt(glm::vec3{ 0.0f, 0.0f, -4.0f }, glm::vec3{ 0.0f, 0.0f, 0.0f }, glm::vec3{ 0.0f, 1.0f, 0.0f });
		u_scene.projMatrix = glm::perspective(45.0f, 1.0f, 0.01f, 10.0f);
		u_scene.viewProjMatrix = u_scene.projMatrix * u_scene.viewMatrix;
		auto u_scene_buf = device.pushDataToUploadBuffer(u_scene, GL::kUniformBufferOffsetAlignment);

		uniforms::Object u_object;
		u_object.modelMatrix = glm::scale(glm::mat4(1.0f), glm::vec3(0.01f));
		auto u_object_buf = device.pushDataToUploadBuffer(u_object, GL::kUniformBufferOffsetAlignment);
